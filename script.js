...
async function loadLevelFile(levelNumber) {
  ...
  try {
    ...
    if (!response.ok) throw new Error("Level não encontrado"+path);
...
async function loadLevel(levelNumber) {
  ...
  patternQueue = currentLevel.pattern ?? null;
  patternRowIndex = 0;
...
function spawnSquare() {
  ...
  // 👉 MODO PATTERN
  if (patternQueue) {
    const row = patternQueue.rows[patternRowIndex];
    if (!row) return;

    row.forEach((cell, laneIndex) => {
      if (cell === 1 && lanes[laneIndex]) {
        const lane = lanes[laneIndex];
        const el = document.createElement("div");

        el.className = `square ${patternQueue.color}`;
        el.style.left = lane.x + "px";
        el.style.top = "-30px";

        game.appendChild(el);
        lane.stack.push({ el, color: patternQueue.color, y: -30 });
      }
    });
    ...
    // terminou o desenho
    if (patternRowIndex >= patternQueue.rows.length) {
      patternQueue = null;
    }
...
setInterval(() => {
  ...
  // se estiver segurando no gradiente, não avança
  if (holding) return;

  loading.textContent = emoji.repeat(count);
  ...
  if (count === max) {
    ...
    setTimeout(() => {
      title.classList.remove('gradient');
      holding = false;
      count = 1;
    }, 700); // 👈 tempo extra no gradiente
  } else {
    title.classList.remove('gradient');
    count++;
  }
